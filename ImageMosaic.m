...
%Concatenate the manmadeImages and naturalImages into a 4 dimentional array
%calls images
...
O = I;
    ...
    for j = 1:size(images, 4) %size(images, 4) is actually the total number of all images
        image = images(:,:,:,j);
        RGBHists = [RGBHists; RGBHist(image)]; %adds a new elemnt into the histogram each time it iterates
    ...
    for j=PATCH_SIZE:PATCH_SIZE:size(I, 1)-2*PATCH_SIZE
        for k=PATCH_SIZE:PATCH_SIZE:size(I, 2)-2*PATCH_SIZE
            ...
            NBPatch = I(j-(PATCH_SIZE-1):j+2*PATCH_SIZE, k-(PATCH_SIZE-1):j+2*PATCH_SIZE, :);
            subImageIndex = getMostSimilarImage(targetPatch, NBPatch, RGBHists, LBPHists, NBTHists);
